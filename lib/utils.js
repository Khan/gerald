...
const getNotified = (
    ...
    if (matches) {
        for (const match of matches) {
            ...
            const names = match.match(/@(Khan\/)?\S*/g);
...
const getFilteredLists = (
    ...
    const actualReviewers = allReviewers.filter(
        (justName: string) => !justName.startsWith('Khan/'),
    );
    const teamReviewers = allReviewers
        .filter((justName: string) => justName.startsWith('Khan/'))
        .map((slugWithKhan: string) => slugWithKhan.slice('Khan/'.length));
